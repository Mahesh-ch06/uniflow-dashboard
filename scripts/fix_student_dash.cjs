...
// 2. Fetch logic
const searchTarget =           setPendingFees("?0");
        }
      } else {
        setPendingFees("?0");
      };
...
const replacement =           setPendingFees("?0");
        ...
        if (coursesData) {
          ...
          const gradedCourses = coursesData.filter(c => c.grade);
...
// the existing text has weird characters due to string replacements or encoding. Let's just use regex.
content = content.replace(/setPendingFees\([^;]+;\s*}\s*} else {\s*setPendingFees\([^;]+;\s*}/g, replacement);
