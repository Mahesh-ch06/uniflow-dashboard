...
code = code.replace(/\\s+<div className=\"text-right\">\\s+<p className=\"text-xs text-muted-foreground\">Total Paid<\\/p>\\s+<p className=\"font-bold\\s+text-success\">?\\{fee\\.paid_amount\\.toLocaleString\\(\"en-IN\"\\)\\} \\/\\s+?\\{totalWithLate\\.toLocaleString\\(\"en-IN\"\\)\\}<\\/p>\\s+<\\/div>\\s+<\\/div>\\s+<\\/div>\\s+<\\/div>/s, replacement);
